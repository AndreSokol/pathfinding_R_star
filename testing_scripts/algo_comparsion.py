...
algos = ['astar', 'jpsearch', 'rstar', 'rjps']
    ...
    for i in range(len(ls)):
        ...
        for alg in algos:
            ...
            if alg == 'rjps':
                ...
                "<numberofsuccessors>" + str( int(2 * 3.1415 * dist[i] * 0.11 * 0.11)) + "</numberofsuccessors>",
                new_map)

                new_map = re.sub(r"<distancetosuccessors>[\w]*</distancetosuccessors>",
                "<distancetosuccessors>" + str(int(dist[i] * 0.11)) + "</distancetosuccessors>",
                new_map)
            else:
                new_map = re.sub(r"<localsearchsteplimit>[\w]*</localsearchsteplimit>",
                "<localsearchsteplimit>" + str(int(dist[i] * 0.28 * 4)) + "</localsearchsteplimit>",
                new_map)

                new_map = re.sub(r"<numberofsuccessors>[\w]*</numberofsuccessors>",
                "<numberofsuccessors>" + str( int(2 * 3.1415 * dist[i] * 0.28 * 5) // 100) + "</numberofsuccessors>",
                new_map)
            ...
            O = open("tests/test_" + str(i) + "_" + alg + ".xml", "w")
            O.write(new_map)
            O.close()
...
def main():
    ...
    stats_file = open("stats_all.csv", "w", newline="", encoding="utf-8", buffering=1)
    stats = csv.writer(stats_file, delimiter=",")

    stats.writerow([""] + algos)
    ...
    for a in algos:
        ...
        for i in range(0, len(ls)):
            print(i+1, "of", len(ls))
            print("../testing_scripts/tests/test_" + str(i) + "_" + a + ".xml")

            for j in range(REPEAT_TIMES):
                print(" ", j + 1, "out of", REPEAT_TIMES)
                try:
                    p = subprocess.run(["../release/PathfindingRStar",
                    "../testing_scripts/tests/test_" + str(i) + "_" + a + ".xml"],
...
def plot():
    z_data = pd.read_csv('stats_all.csv', index_col=0)
